...
const getChartSpec = (i: number, showLeftAxis: boolean) => {
  const monthList = [
    ['Paid-Ads', 'Preload', 'Sharing', 'SEO', 'Referral', 'Organic&Others'],
    ['Push', 'Sharing', 'SEO', 'Incentive', 'EDM'],
    ['Lite', 'Coin', 'Pad'],
    ['M10n', 'External-ban/unban', 'Applog Upgrade', 'Rec', 'Content&Product']
  ];
  const dataList = [
    [
      [22.0, 33.8],
...
export const SomeCharts = () => {
  ...
  useEffect(() => {
    ...
    [
      ['User Acquisition + 35.6M', '(21.3%)'],
      ['User Acquisition + 14.9M', '(8.9%)'],
      ['User Acquisition + 34.9M', '(20.9%)'],
      ['User Acquisition + 81.7M', '(48.9%)']
    ].forEach((item, index) => {
      let left = dataCountList.slice(0, index).reduce((a, b) => {
        return a + b * 100;
      }, 0);
