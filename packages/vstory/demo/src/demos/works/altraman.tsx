import { Player } from '../../../../../vstory-core/src';
...
function addTimeLine(story: Story) {
  ...
  story.addCharacter(
    {
      ...
      id: `timeline_bg`,
      zIndex: 1,
      position: { x: startPos.x, y: startPos.y, width: 1715, height: 200 },
      options: {
        graphic: {
          fill: 'white',
          cornerRadius: 10
  ...
  data.forEach(({ year, text }, index) => {
    addYearComponent(story, Number(year), index, text, startPos, 'horizontal', false);
  });
}
...
function addYearComponent(
  ...
  if (direction === 'horizontal') {
    pos.x = startPos.x + 250 + index * yearDistance * (revertLayoutSide ? -1 : 1);
    pos.y = startPos.y + 100;
  } else {
...
export const Altraman = () => {
  const CONTAINER_ID = 'altraman';
  ...
  return (
    <div style={{ width: '1920px', height: '1080px', position: 'relative' }} id={CONTAINER_ID}>
      <canvas
        id="altraman-canvas"
        style={{ width: 1920, height: 1080, position: 'absolute', top: 0, left: 0, zIndex: 1 }}
      />
    </div>
  );
};
