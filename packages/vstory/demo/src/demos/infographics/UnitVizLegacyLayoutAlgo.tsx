...
const manInSuicideData: UnitNode[] = [
  {
    ...
    count: 200,
    children: [
      { category: 'man', color: '#3164b4', count: 30 },
      { category: 'not-man', count: 170 }
    ]
  },
  {
    category: 'not-suicide',
...
function createUnitMatrix(unitData: UnitNode[], sceneNumber: number) {
  ...
  const radius = Math.min(vizWidth / (numCols * 3 - 1), (vizHeight + vizMargin.y) / 2);
  const gap = radius;

  const startX = (screenWidth - radius * (numCols * 3 - 1)) / 2;
...
export const UnitVizLegacyLayoutAlgo = () => {
