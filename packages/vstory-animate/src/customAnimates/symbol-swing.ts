import { ACustomAnimate, createLine, generatorPathEasingFunc } from '@visactor/vrender';
...
export class SymbolSwing extends ACustomAnimate<{ x?: number; y?: number }> {
  ...
  onBind(): void {
    if (this.target) {
      const attrs = this.target.attribute as any;
      this.target.setAttributes({ ...this.from, y: attrs._layoutHeight });
    }
    if (this.showLine) {
