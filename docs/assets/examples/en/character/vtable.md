...
`VTable` is a generic Character element that allows you to create various types of tables. You can visit the [VTable official website](/vtable) for more information on table types. Configure the table as desired, and then place it in a VStory for further definition and configuration.
Set `type: 'VTable'` to use this table as a Character.
