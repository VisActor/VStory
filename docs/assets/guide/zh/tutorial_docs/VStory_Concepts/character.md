...
- type：`character`的类型，目前 VStory 支持的`character`类型包括但不限于：`VChart`、`Text`、`Image`、`Path`等
...
### 基本组件（Image、Line、Rect、Shape、Text、Arc、Polygon、Path）

组件类型的接口定义如下，其中基本组件（Image、Line、Rect、Shape、Text、Arc、Polygon、Path）的配置都直接基于 VRender 的对应图元，配置在 graphic 属性上：
...
- Path 是基于 VRender 的[Path 图元](/vrender/option/Path)
...
const story = new VStory.Story(null, { dom: CONTAINER_ID, background: '#ebecf0', width: 400, height: 520 });
const player = new VStory.Player(story);
story.init(player);
